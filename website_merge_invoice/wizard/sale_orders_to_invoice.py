...
class SaleOrdersToInvoice(models.TransientModel):
    _name = "sale.orders.to.invoice"
    _description = "Sale Orders To Invoice"
    ...
    def create_invoices(self):
        ...
        if any(order.invoice_status != 'to invoice' for order in sale_orders):
            raise UserError(_("You cannot invoice an order which is not in 'To Invoice' state."))

        sale_orders_all = sale_orders.filtered(lambda order: not order.to_create_invoice)
        sale_orders_individual = sale_orders - sale_orders_all
        
        if self.advance_payment_method == 'delivered':
            if sale_orders_all:
                sale_orders_all.with_context(partner=self.partner_id).action_invoice_create_all()
            for order in sale_orders_individual:
                order.action_invoice_create()
        elif self.advance_payment_method == 'all':
            if sale_orders_all:
                sale_orders_all.with_context(partner=self.partner_id).action_invoice_create_all(final=True)
            for order in sale_orders_individual:
        ...
        payment_transactions = self.env['payment.transaction'].sudo().search([('sale_order_id','in',sale_orders_all.ids)])
        payment_transactions.write({'partner_id':self.partner_id.id})
